...
Current state:
- `.venv\Scripts\python.exe -m pytest -v` → **133/133 passing.**
...
- In progress: a "Holdings Review" feature (grounded, LLM-narrated analysis of existing mutual fund holdings vs. a computed benchmark) — plan at `~/.claude/plans/deep-rolling-lamport.md`. Phase A landed: CAS parser now captures mutual fund ISIN (was silently discarded before), and new `app/modules/amfi_nav_client.py`/`mfapi_client.py`/`market_data_client.py` resolve an ISIN to AMFI scheme data + trailing returns — not yet wired into any endpoint. Open question before Phase B: whether Render's servers can actually reach `amfiindia.com` (the user's home network in Denmark cannot — confirmed via two independent HTTP stacks; a temporary `/api/debug/amfi-check` endpoint was added to test this from the deployed backend and should be removed once answered).
