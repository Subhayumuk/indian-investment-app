...
## Where things stand (updated 2026-09-01)
...
Current state:
- `.venv\Scripts\python.exe -m pytest -v` → **208/208 passing.**
...
- "Holdings Review" feature (grounded, LLM-narrated analysis of existing mutual fund holdings vs. a computed benchmark) — plan at `~/.claude/plans/deep-rolling-lamport.md`. **Phase A and Phase B both landed and are verified end-to-end against live data** (see the 2026-09-01 entry below); Phase C (AI narration) and Phase D (a button on the results page) are not started.
...
- **2026-09-01 session: Holdings Review Phase B + a real cross-country bug.** Built `app/modules/benchmark_engine.py` (peer benchmark, reusing `RecommendationEngine._build_portfolio_health` rather than duplicating it) and `app/modules/holdings_review_engine.py` (per-fund verdicts — `aligned`/`worth_reviewing`/`underperforming_category`/`overconcentrated`/`data_unavailable` — from real AMFI/mfapi.in data vs. an asset-class benchmark), exposed as `POST /api/holdings-review` (reuses the existing `UserProfile` request shape). Verified live against the user's real 16-fund CAS statement: 16/16 matched, real trailing returns, ~12s response. Two real bugs found via that live test, both fixed: (1) fund lookups ran sequentially, so wait time scaled with fund count — now concurrent via `asyncio.gather`, with a new lock in `amfi_nav_client.py`'s `AmfiNavClient` so concurrent lookups on a cold cache share one ~10MB fetch instead of each triggering their own; (2) every suggested instrument's tax note was hardcoded Danish-only (`danish_tax_note`, always populated, always shown) regardless of the user's actual country — a pre-existing bug, not introduced today. Renamed to `residence_tax_note` and made genuinely country-aware via `instrument_catalog.residence_tax_note(country, instrument_type)` (one hand-written note per supported country, deliberately quoting only destination-country/DTAA-treaty rates, never India's own Budget-driven domestic rates) — wired into both new-money instrument suggestions and Holdings Review's existing-fund verdicts. 208/208 tests passing.
...
- **`app/modules/`** — the engine, split by concern: `residency_engine.py` (tax residency determination), `eligibility_checker.py` (what an NRI from a given country can actually invest in), `tax_engine.py` (India + foreign-country tax treatment, DTAA), `allocation_engine.py` (risk/horizon → asset allocation), `instrument_catalog.py` (the instrument list itself, plus `residence_tax_note()` for country-aware tax notes), `confidence_scorer.py`, `explanation_builder.py`, orchestrated by `recommendation_engine.py`. Holdings Review adds `amfi_nav_client.py`/`mfapi_client.py`/`market_data_client.py` (ISIN → real fund data, Phase A), `benchmark_engine.py` + `holdings_review_engine.py` (peer benchmark + per-fund verdicts, Phase B), exposed via `app/api/holdings_review.py` (`POST /api/holdings-review`) — a fully separate, additive path from `recommendation_engine.py`'s own flow.
